...
<p align="center">
jfx-maven-plugin 是一个 JavaFX 打包插件，提供了一体化方案来解决基于 JavaFX 框架打包为独立桌面应用程序的难题。插件会自动检测项目是否为模块化项目，并选择对应的打包方式。支持三种打包模式（非模块化、模块化、预构建运行时镜像），一键打包全平台（Windows、macOS、Linux）可执行文件（app-image、exe、msi、rpm、deb、pkg、dmg），内置 jdeps 依赖分析功能，自动精简 JRE。
</p>
...
插件支持三种打包模式，自动检测项目类型并选择对应的打包方式：

| 模式 | 触发条件 | 说明 |
|------|---------|------|
| **非模块化模式** | 无 `module-info.class` | 自动复制 JAR 到 lib 目录，使用 `--input`/`--main-jar`/`--main-class`，支持 `addModules`/`jlinkOptions` 精简 JRE |
| **模块化模式** | 有 `module-info.class` | 使用 `--module-path`/`--add-modules`/`--module`，自动解析模块路径，自动精简 JRE |
| **预构建运行时模式** | 配置了 `imagePath` | 使用 `--runtime-image`/`--module`，基于预先构建的运行时镜像打包 |

### 4. jdeps 依赖分析

执行 `mvn jfx:jdeps` 分析项目所有依赖并输出所需的 JDK 模块名（逗号分隔），可用于精简非模块化项目的 JRE。

```shell
mvn jfx:jdeps
# 输出示例: java.base,java.compiler,java.desktop,java.management,java.prefs,java.scripting
```

将输出结果配置到 `addModules` 中，配合 `jlinkOptions` 使用可显著减小打包体积：

```xml
<configuration>
    <addModules>java.base,java.compiler,java.desktop,java.management,java.prefs,java.scripting</addModules>
    <jlinkOptions>
...
**jdeps 参数：**
- `multiRelease`：多版本 JAR 的版本号，不设置则自动检测当前 Java 版本
    ...
    <plugins>
        <plugin>
            ...
            <configuration>
                ...
                <!-- 使用 jdeps 分析结果配置 addModules -->
                <addModules>java.base,java.desktop,java.scripting,java.logging,java.management,java.naming,java.net.http,java.sql,java.xml,jdk.zipfs</addModules>
                <jlinkOptions>
                    <option>--strip-native-commands</option>
                    <option>--strip-debug</option>
                    <option>--no-man-pages</option>
...
<plugin>
    ...
    <configuration>
        ...
        <!-- ===== 模块化 / JRE 精简配置 ===== -->
        <addModules>java.base,java.desktop,java.sql</addModules>
        <jmodsPath>D:\develop\jdk-21\jmods</jmodsPath>
        <jlinkOptions>
            <option>--strip-debug</option>
            <option>--compress</option>
            <option>zip-9</option>
        ...
        <modulePath>
            <arg>/path/to/module1</arg>
            <arg>/path/to/module2</arg>
        </modulePath>
...
| `imagePath` | `String` | 否 | — | `--runtime-image` | 预构建的运行时镜像路径，指定后启用预构建运行时模式 |
| `options` | `List<String>` | 否 | — | `--java-options` | 传递给 JVM 的选项列表 |
| `arguments` | `List<String>` | 否 | — | `--arguments` | 传递给主类的命令行参数列表 |

### 模块化 / JRE 精简参数

| 参数 | 类型 | 默认值 | 对应 jpackage 选项 | 说明 |
|------|------|--------|-------------------|------|
| `addModules` | `String` | — | `--add-modules` | 要包含在运行时镜像中的额外 JDK 模块，逗号分隔。可通过 `mvn jfx:jdeps` 分析获得 |
| `jlinkOptions` | `List<String>` | — | `--jlink-options` | 传递给 jlink 的选项列表，用于控制运行时镜像生成。与 `imagePath` 互斥 |
| `modulePath` | `List<String>` | — | `--module-path` | 额外的模块路径列表 |
| `jmodsPath` | `String` | 自动检测 | — | JDK jmods 目录路径，用于模块化打包时定位 JDK 模块 |
...
| `appVersion` | `String` | `${project.version}` | `--app-version` | 应用版本号（建议使用三段式版本如 `1.0.1`） |
...
4. **模块检测**（非 imagePath 模式）：使用 plexus-java 自动检测项目是否包含 `module-info.class`
   - **模块化项目**：解析模块路径，分离 modulepath 和 classpath 元素
   - **非模块化项目**：复制所有运行时依赖 JAR 和主 JAR 到 `target/javafx/lib/`
5. **构建命令**：根据模块化状态和配置参数组装完整的 `jpackage` 命令行
6. **执行打包**：调用 `jpackage` 命令完成打包
7. **重命名构建物**：自动添加平台和架构信息（如 `{name}-windows-x86_64-{version}.exe`）
8. **清理临时文件**：删除打包完成后不再需要的 `lib/` 目录
